...
public final class ClassUtils {
    ...
    public static <T> T newInstance(Class<T> clazz) {
        try {
            ...
            throw new RuntimeException(StrUtil.format("实例化对象时出现错误,请尝试给 %s 添加无参的构造方法", e, clazz.getName()));
        }
    }
    ...
    public static Class<?> toClassConfident(String name, ClassLoader classLoader) {
        try {
            ...
            throw new RuntimeException(StrUtil.format("找不到指定的class！请仅在明确确定会有 class 的时候，调用该方法", e));
        }
    }
    ...
    public static String getPackageName(String fqClassName) {
        ...
        int lastDotIndex = fqClassName.lastIndexOf(StrUtil.DOT);
        return (lastDotIndex != -1 ? fqClassName.substring(0, lastDotIndex) : StrUtil.EMPTY);
    }
